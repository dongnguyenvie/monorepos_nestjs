import { ICoreService } from '@noinghe/shared/dist/interfaces';
import { IListPayload } from '@modules/users/interface';
import { ReqUser } from '@shared/dtos';
import { CoreEntity } from '@shared/entities';
import { SvUnknownError } from '@shared/errors';
import { paginationHelper } from '@shared/utils';
...
export class CoreService<E extends CoreEntity> implements ICoreService {
  ...
  async softDeleteById(id: string, reqUser?: ReqUser) {
    try {
      ...
      Object.assign(updated, { deletedAt: new Date(), ...(!!reqUser ? { deletedBy: reqUser.id } : {}) });
